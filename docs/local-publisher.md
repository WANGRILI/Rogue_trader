...
结果发布器与分析流程相互独立。它只在检测到完整结果后读取结构化决策，先追加决策主表，再让委托明细表、本地消息包、飞书群消息和飞书普通电子表格消费刚刚落盘的同一条决策记录。所有发布步骤都不调用 LLM。
...
`参数化委托.csv` 是从已校验 `执行计划.json` 派生的审计视图。它把每个场景中的每条委托展开为一行；无委托场景也以 `row_type=no_order` 保留。v2 额外保存结构化触发器、委托生成时间及来源、回测有效期和风险上限。委托时间只接受最终决策的真实生成时间；异常历史修复若无匹配快照会被研究门禁隔离，不再生成虚构回退时间。稳定的 `order_event_id` 防止重复追加，旧 v1 CSV 会从原始计划安全升级。

## 完整性和幂等性

- `运行清单.json` 记录运行全生命周期；只有同时存在 `运行索引.json` 和 `最终决策.json` 的成功目录才可发布。
- `运行索引.json` 是写出流程最后创建的完成标记。
- 索引与决策中的标的、日期和动作必须一致。
- `INCOMPLETE` 结果不会发布。
- 开发或手动运行默认是候选结果；同一分析日期和标的只自动认定一个正式结果。候选结果必须显式使用 `publish --promote` 才能提升。
...
- 决策主表失败时，委托明细、本地消息、飞书群消息和飞书表格均不会执行。主表成功后，各目标分别记录状态和重试，任一失败都不会回滚主表或重新运行付费分析。
- 启用飞书群消息后，有效的 `执行计划.json` 与决策摘要会合并为一张卡片，按“指令在前、决策在后”展示；两部分共用一个投递状态和重试任务，不增加模型调用。
